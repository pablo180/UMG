$(document).ready(function () {
    ...
    $("#BtnAgregarProducto").click(function () {
        if ($("#DropProducto").val() != "" && $.isNumeric($("#TxtCantidad").val()) && $.isNumeric($("#TxtPrecio").val())) {
            ...
            var Vehiculo;
            if ($("#DivVehiculo").is(":visible")) {
                Vehiculo = $("#DropVehiculo option:selected").html();
            } else {
                Vehiculo = "N/A";
            }
            $(".FacturaDetalle tbody").append("<tr><td>" + $("#DropProducto").val() + "</td><td>" + $("#DropProducto option:selected").html() + "</td><td>" + $("#TxtCantidad").val() + "</td><td>" + $("#TxtPrecio").val() + "</td><td>" + total + "</td><td>" + Vehiculo + "</td><td class='table-action'><a class='delete-row baja'><i class='fa fa-trash-o'></i></a></td></tr>");
        } else {
            jQuery.gritter.add({
                title: 'Se Ha Producido Un Error!',
    ...
    $("#BtnGuardarFacturaViaticos").click(function (event) {
        ...
        $.ajax({
            ...
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            data: "{'Versatec':'" + $("#DropTipoLiquidacion").val() + "','FechaFactura':'" + $("#TxtFecha").val() + "','TipoDocumento': '" + $("#DropTipoDocumento").val() + "', 'OrdenTrabajo':'" + $("#TxtOrdenTrabajo").val() + "', 'NitProveedor':'" + $("#TxtNitProveedor").val() + "', 'NombreProveedor': '" + $("#TxtNombreProveedor").val() + "', 'SerieFactura': '" + $("#TxtSerieFactura").val() + "', 'NumeroFactura':'" + $("#TxNumeroFactura").val() + "', 'Observacion': '" + $("#TxtObservacion").val() + "', 'Detalle':" + JSON.stringify(detalle) + "}",
            success: function (data) {
                $.each(data.d, function (index, item) {
                    if (item.StrError == "0") {
                        jQuery.gritter.add({
